...
public class Pc_fragment_espera extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        View view = LayoutInflater.from(getContext()).inflate(R.layout.fragment_pc_espera,container,false);
    ...
    private void mostrarUsuariosMnt(){
        Call<List<Ticket>> call = ApiClient.getClient().create(IApiTicket.class).getTicketsEspera();
        call.enqueue(new Callback<List<Ticket>>() {

            @Override
            public void onResponse(Call<List<Ticket>> call, Response<List<Ticket>> response) {
                if(response.isSuccessful()){
                    ticketEsperaList = response.body();
                    adapter = new AdapterTickets(ticketEsperaList, Pc_fragment_espera.this.getContext(), new AdapterTickets.OnItemClickListener() {
                        @Override
            ...
            public void onFailure(Call<List<Ticket>> call, Throwable t) {
                Toast.makeText(Pc_fragment_espera.this.getContext(),"ERROR DE CONEXION",Toast.LENGTH_SHORT).show();
            }
        });
    }
}